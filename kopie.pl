...
droga(A,B,[A|W]):-next(A,Z,_),droga(Z,B,W).
ostatni([A|[]],A).
ostatni([_|B],X):-ostatni(B,X).
przedostatni([A|[_|[]]],A).
przedostatni([_|B],X):-przedostatni(B,X).
kopia([],[]).
kopia([X|Y],[X|Z]):-kopia(Y,Z).
kopiabezost([_|[]],[]).
kopiabezost([X|Y],[X|Z]):-kopiabezost(Y,Z).
